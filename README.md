...
(0.6.5, 2026-07-13) pins `github.com/hashicorp/nomad v1.11.3` in its `go.mod`
and its changelog's `UNRELEASED` section is empty. It shipped two months
*after* Nomad 2.0.1, so 2.0 was available and not targeted — that is a signal
about the maintainers' priorities, not a compatibility finding. On a
podman-backed fleet the driver is the execution substrate, so it — not the
server — sets the ceiling.

The default is **2.0.5**, and the security argument is stronger than
"gets no future patches" — the community 1.x line is *already* missing
fixes. Nomad 2.0.1 (2026-05-12) carries two CVEs:

| CVE | Fix |
|---|---|
| **CVE-2026-6959** | logging FIFO symlink-swap attacks — mitigated by making the alloc logs directory a read-only bind mount |
| **CVE-2026-7474** | dynamic host volumes: code execution outside the plugin directory |

1.11.3 (2026-03-11) predates both, and there is no community 1.11.4+ to
backport into: those releases are `+ent` only and the community
`linux_amd64.zip` 404s. So 1.x is not a patched-but-frozen fallback — it
is unpatched today.

Against that, the driver's lack of a 2.0 *declaration* turns out not to
...
- The plugin handshake — `ProtocolVersion`, `MagicCookieKey`,
  ...
  between Nomad v1.11.3 and v2.0.5. A driver built against 1.11.3 loads and
  handshakes with a 2.0.5 client.
- `plugins/drivers/proto/driver.proto` changed **additively only** between
  1.11.3 and 2.0.5: two new RPCs (`Init`, `Shutdown`), two new fields, four
...
> unhealthy driver.
>
> This is the CVE-2026-6959 mitigation, so if it bites, the fix is to stop
> writing there — not to roll back to a Nomad that still has the
> vulnerability.
